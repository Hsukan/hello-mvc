	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			if(member != null && password.equals(member.getPassword())) {
				...
				//saveId 처리
				//cookie vs webStroage -> 공통점 : 클라이언트에 정보 저정 -> 차이점 : 쿠키는 서버로 무조건 전송
				...
				if(saveId != null) {
					...
					//유효기간(초 단위)
					cookie.setMaxAge(7*24*60*60); // 7일(초단위)
				}
				//saveId를 사용하지않는 경우
				else {
			...
			//로그인 실패(아이디가 존재하지 않는 경우 || 비밀번호를 잘못 입력한 경우)
			else {
			...
			//4. 응답 처리 : 로그인 후 url변경을 위해 리다이렉트 처리!
			//응답 302 redirect 전송. 
			//브라우져에게 location으로 재요청을 명령.
			response.sendRedirect(request.getContextPath() + "/"); // /mvc/ 마지막에 슬래쉬로 닫아줘야 깔끔하게 처리 
		} catch (IOException e) {
			e.printStackTrace(); //로깅
			throw e; // tomcat에게 예외 던짐/ -> 에러페이지로 이동
		}
